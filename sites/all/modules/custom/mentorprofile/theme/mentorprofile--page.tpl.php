    ...
    <div id="page">
        ...
        <div id="content" class="clearfix" style="min-height:600px">
            ...
            <div id="main-content" class="container clearfix" style="height:auto">
                    ...
                    <div class="button790 one-fourth" style="height:120px; width:120px; padding-right:11px">
                   <?php endif; ?>
                      <?php  
                    if ($tabs["#primary"] == ""): ?>
...
">
	...
	<span style=" text-align: center; display: none; position: absolute; width: 120px; height: 120px; top: 0; left: 0; background: rgba(22, 22, 32, 0.44);">
				...
				</span></div><div style="margin-left: 10px;">
           <?php 			
		   		$rating = mentoringcommon_mentor_rating($user_id);  		   		
		   ...
		   $path = drupal_get_path('module', 'fivestar');	
		  	...
		  	$settings = array(
		   	...
		   	//print render(drupal_get_form('fivestar_custom_widget', $rating, $settings));
				//$fivestar_values = fivestar_get_votes('node', 1, 'vote', $account->mid);
			//if($showRating) {			
			  //print render(drupal_get_form('fivestar_custom_widget', $rating, $settings)); 
			//}
			
		   ?>    </div> <a style="padding: 10%; color: #FE7D00; font-size: 14px;" href="<?php print '/user/' . $user_id?>">View Reviews
		   </a>    
		   ...
		   <?php 
			if($rateMentor) {
		   ?>
				<a href="<?php print '/mentor/' . $user_id . '/rate'?>">Write a Review</a>
		   <?php
			}
			   ...
			   $settings = array(
				...
				//$fivestar_values = fivestar_get_votes('node', 1, 'vote', $account->mid);
			if($showRating) {
			  // print render(drupal_get_form('fivestar_custom_widget', $values, $settings)); 
			}
			   ?>
                ...
                <div class="user-details-para">
               	<div>
                	...
                	<?php $mentorProfile = mentoringcommon_mentor_details($user->uid);
                	echo ucwords($mentorProfile->first_name) ?> <?php echo ucwords($mentorProfile->last_name) ?>
                	</b>
                </div> 
